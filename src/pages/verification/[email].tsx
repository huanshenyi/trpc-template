...
import { between10m } from '~/utils/format';

type Input = inferProcedureInput<AppRouter['user']['verificationEmail']>;
...
const VerificationPage: NextPageWithLayout = () => {
  ...
  const verificationEmail = trpc.user.verificationEmail.useMutation({
    async onSuccess() {
      setStatus('ご協力感謝します');
    },
  });
  const handelEmailVerified = useCallback(async (input: Input) => {
    const code = query['code'];
    const intCode = parseInt(code as string, 10);
    if (code && between10m(dayjs(date), dayjs(intCode))) {
      try {
        await verificationEmail.mutateAsync(input);
      } catch (cause) {
  ...
  useEffect(() => {
    ...
    handelEmailVerified(input);
  }, [email, handelEmailVerified]);
    ...
    <div className="w-full h-screen flex flex-col items-center justify-center">
      ...
      <div className="flex flex-col items-center justify-center">
        ...
        <p className="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-600 mt-2">
          {verificationEmail.isError ? '認証失敗です' : '認証完了しました'}
        </p>
        <p className="md:text-lg xl:text-xl text-gray-500 mt-4">{status}</p>
      </div>
    </div>
  );
};
